...
サイズがNodeの分あるデータの先頭アドレスを指すｐNodeに代入する
ｐNodeが指すデータcityにnewcityを代入する
ｐNodeの次のアドレスを指すｐNextにｐNextを代入する
return(pNode)で先頭アドレスを出力する


2つ目
iとjともにー１とおく
ｊをすべて見終わったとき終了するようにi１づつ足していく
pListの中にあるcity.idとkeyと同じであるIDを比較して同じであればiにjを代入してwhileから出る、ならない場合は比較を続ける
一致する場合はiにjが入り一致しなかった場合はー１が入っている

チャレンジ
要素を全て確認
CurrentNodeがNULLとなった場合にはfor文を抜けて-1を返す。
引数のIDと一致すればcityの要素を代入し、何番目の要素かを返す。

文字列の比較を行うためにstrcmpを導入
一致する場合は0がかえるのでif文を使い、一致する場合はindexとiを返し、見つからない場合は-1を返す。
